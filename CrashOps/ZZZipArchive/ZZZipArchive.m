...
NSString *const ZZZipArchiveErrorDomain = @"ZZZipArchiveErrorDomain";
...
@interface NSData(ZZZipArchive)
- (NSString *)_base64RFC4648 API_AVAILABLE(macos(10.9), ios(7.0), watchos(2.0), tvos(9.0));
- (NSString *)_hexString;
@end

@interface NSString (ZZZipArchive)
- (NSString *)_sanitizedPath;
@end

@interface ZZZipArchive ()
- (instancetype)init NS_DESIGNATED_INITIALIZER;
@end

@implementation ZZZipArchive
{
    /// path for zip file
    NSString *_path;
    zipFile _zip;
...
+ (BOOL)isPasswordValidForArchiveAtPath:(NSString *)path password:(NSString *)pw error:(NSError **)error {
    ...
    if (zip == NULL) {
        if (error) {
            *error = [NSError errorWithDomain:ZZZipArchiveErrorDomain
                                         code:ZZZipArchiveErrorCodeFailedOpenZipFile
        ...
        do {
            ...
            if (ret != UNZ_OK) {
                if (ret != MZ_PASSWORD_ERROR) {
                    if (error) {
                        *error = [NSError errorWithDomain:ZZZipArchiveErrorDomain
                                                     code:ZZZipArchiveErrorCodeFailedOpenFileInZip
            ...
            if (ret != UNZ_OK) {
                if (error) {
                    *error = [NSError errorWithDomain:ZZZipArchiveErrorDomain
                                                 code:ZZZipArchiveErrorCodeFileInfoNotLoadable
                ...
                if (readBytes < 0) {
                    ...
                    if (readBytes != Z_DATA_ERROR) {
                        if (error) {
                            *error = [NSError errorWithDomain:ZZZipArchiveErrorDomain
                                                         code:ZZZipArchiveErrorCodeFileContentNotReadable
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(nullable id<ZZZipArchiveDelegate>)delegate
{
          ...
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError **)error
               delegate:(nullable id<ZZZipArchiveDelegate>)delegate
{
      ...
      completionHandler:(void (^_Nullable)(NSString *path, BOOL succeeded, NSError * _Nullable error))completionHandler
{
    ...
    if (path.length == 0 || destination.length == 0)
    {
        ...
        NSError *err = [NSError errorWithDomain:ZZZipArchiveErrorDomain code:ZZZipArchiveErrorCodeInvalidArguments userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (zip == NULL)
    {
        ...
        NSError *err = [NSError errorWithDomain:ZZZipArchiveErrorDomain code:ZZZipArchiveErrorCodeFailedOpenZipFile userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    if (ret != UNZ_OK && ret != MZ_END_OF_LIST)
    {
        ...
        NSError *err = [NSError errorWithDomain:ZZZipArchiveErrorDomain code:ZZZipArchiveErrorCodeFailedOpenFileInZip userInfo:userInfo];
        if (error)
        {
            *error = err;
        }
    ...
    do {
        ...
        @autoreleasepool {
            ...
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:@"ZZZipArchiveErrorDomain" code:ZZZipArchiveErrorCodeFailedOpenFileInZip userInfo:@{NSLocalizedDescriptionKey: @"failed to open file in zip file"}];
                success = NO;
                break;
            }
            ...
            ret = unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if (ret != UNZ_OK) {
                unzippingError = [NSError errorWithDomain:@"ZZZipArchiveErrorDomain" code:ZZZipArchiveErrorCodeFileInfoNotLoadable userInfo:@{NSLocalizedDescriptionKey: @"failed to retrieve info for file"}];
                success = NO;
                unzCloseCurrentFile(zip);
                break;
            }
            ...
            NSString * strPath = [ZZZipArchive _filenameStringWithCString:filename
            ...
            if (err != nil) {
                ...
                NSLog(@"[ZZZipArchive] Error: %@", err.localizedDescription);
            }
                ...
                if (readBytes >= 0) {
                        ...
                        if (readBytes > 0) {
                            if (0 == fwrite(buffer, readBytes, 1, fp)) {
                                if (ferror(fp)) {
                                    ...
                                    NSLog(@"[ZZZipArchive] %@", message);
                                    success = NO;
                                    unzippingError = [NSError errorWithDomain:@"ZZZipArchiveErrorDomain" code:ZZZipArchiveErrorCodeFailedToWriteFile userInfo:@{NSLocalizedDescriptionKey: message}];
                                    break;
                                }
                            }
                        } else {
                        ...
                        if (nestedZipLevel
                            ...
                            if (fileInfo.mz_dos_date != 0) {
                                ...
                                if (attr) {
                                    if (![fileManager setAttributes:attr ofItemAtPath:fullPath error:nil]) {
                                        ...
                                        NSLog(@"[ZZZipArchive] Failed to set attributes - whilst setting modification date");
                                    }
                                }
                            }
                            ...
                            if (permissions != 0) {
                                ...
                                if (![fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil]) {
                                    ...
                                    NSLog(@"[ZZZipArchive] Failed to set attributes - whilst setting permissions");
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        switch (errnoSave) {
                                ...
                                // Just log the error
                            {
                                NSError *errorObject = [NSError errorWithDomain:NSPOSIXErrorDomain
                                                                           code:errnoSave
                                                                       userInfo:nil];
                                NSLog(@"[ZZZipArchive] Failed to open file on unzipping.(%@)", errorObject);
                            }
                                break;
                        }
                        ...
                        if (isSeriousError) {
                            ...
                            unzCloseCurrentFile(zip);
                            // Log the error
                            NSLog(@"[ZZZipArchive] Failed to open file on unzipping.(%@)", unzippingError);
                ...
                if (overwrite)
                {
                    if ([fileManager fileExistsAtPath:fullPath])
                    {
                        ...
                        if (!removeSuccess)
                        {
                            ...
                            NSLog(@"[ZZZipArchive] %@", message);
                            success = NO;
                            unzippingError = [NSError errorWithDomain:ZZZipArchiveErrorDomain code:localError.code userInfo:@{NSLocalizedDescriptionKey: message}];
                        }
                    }
                }
    ...
    if (success && preserveAttributes) {
        ...
        for (NSDictionary * d in directoriesModificationDates) {
            if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: [d objectForKey:@"modDate"]} ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
                NSLog(@"[ZZZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
            }
            if (err) {
                NSLog(@"[ZZZipArchive] Error setting directory file modification date attribute: %@", err.localizedDescription);
            }
        }
    }
    ...
    if (crc_ret == MZ_CRC_ERROR)
    {
        ...
        retErr = [NSError errorWithDomain:ZZZipArchiveErrorDomain code:ZZZipArchiveErrorCodeFileInfoNotLoadable userInfo:userInfo];
    }
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory {
    return [ZZZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:keepParentDirectory withPassword:nil];
}

+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray<NSString *> *)paths withPassword:(NSString *)password
{
    ZZZipArchive *zipArchive = [[ZZZipArchive alloc] initWithPath:path];
...
+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath withPassword:(nullable NSString *)password {
    return [ZZZipArchive createZipFileAtPath:path withContentsOfDirectory:directoryPath keepParentDirectory:NO withPassword:password];
}


+ (BOOL)createZipFileAtPath:(NSString *)path withContentsOfDirectory:(NSString *)directoryPath keepParentDirectory:(BOOL)keepParentDirectory withPassword:(nullable NSString *)password {
    return [ZZZipArchive createZipFileAtPath:path
...
//                NSLog(@"[ZZZipArchive] the archive path and the file path: %@ are the same, which is forbidden.", fullFilePath);
//                continue;
//            }
...
- (BOOL)writeFolderAtPath:(NSString *)path withFolderName:(NSString *)folderName withPassword:(nullable NSString *)password
{
    ...
    [ZZZipArchive zipInfo:&zipInfo setAttributesOfItemAtPath:path];
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    zip_fileinfo zipInfo = {};
    [ZZZipArchive zipInfo:&zipInfo setDate:[NSDate date]];
...
- (BOOL)close
{
    NSAssert((_zip != NULL), @"[ZZZipArchive] Attempting to close an archive which was never opened");
...
{
    NSCalendar *currentCalendar = ZZZipArchive._gregorian;
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime
{
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[ZZZipArchive] MSDOS date masks don't add up");
...
@implementation NSData (ZZZipArchive)
...
@implementation NSString (ZZZipArchive)
