...
#ifndef _ZZZIPARCHIVE_H
#define _ZZZIPARCHIVE_H
...
extern NSString *const ZZZipArchiveErrorDomain;
typedef NS_ENUM(NSInteger, ZZZipArchiveErrorCode) {
    ZZZipArchiveErrorCodeFailedOpenZipFile      = -1,
    ZZZipArchiveErrorCodeFailedOpenFileInZip    = -2,
    ZZZipArchiveErrorCodeFileInfoNotLoadable    = -3,
    ZZZipArchiveErrorCodeFileContentNotReadable = -4,
    ZZZipArchiveErrorCodeFailedToWriteFile      = -5,
    ZZZipArchiveErrorCodeInvalidArguments       = -6,
};

@protocol ZZZipArchiveDelegate;

@interface ZZZipArchive : NSObject
...
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError * *)error
               delegate:(nullable id<ZZZipArchiveDelegate>)delegate NS_REFINED_FOR_SWIFT;
...
@protocol ZZZipArchiveDelegate <NSObject>
